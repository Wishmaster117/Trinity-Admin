...
class download implements download_interface
{
	...
	{
		// Ajout pour éviter les warnings Undefined variable
        // $new_real_file = '';
        // $file_size     = 0;
		switch ($module)
		{
		...
		/* Ajout bloque le uploads aux non admins */
		$external_only          = (int) $this->config['dlext_external_only'];
		if ($external_only && !$file_extern && !$this->dlext_auth->user_admin())
		{
	...
	public function dl_edit_download($module, $df_id = 0, $own_edit = 0, $u_action = '')
	{
		...
		// Montrer le champ d'upload si :
		//   - l’option “external only” est désactivée
		//   - OU l’utilisateur est administrateur
		$show_upload = !$this->config['dlext_external_only'] || $this->dlext_auth->user_admin();
		$this->template->assign_var('S_SHOW_UPLOAD', $show_upload);
		// Fin Ajout
		$this->template->assign_vars($template_ary);
		// Transmettre au template si l'utilisateur est administrateur
		// $this->template->assign_var('S_USER_IS_ADMIN', $this->dlext_auth->user_admin());
