...
function NPCModule:CreateNPCPanel()
    ...
    navPageLabel:SetPoint("BOTTOM", npc, "BOTTOM", 0, 42)
	...
	TrinityAdmin.AutoSize(inputBox, 20, 16)
	...
	TrinityAdmin.AutoSize(actionButton, 20, 16)
    ...
    for _, cmd in ipairs(fullCommands) do
        ...
        for _, field in ipairs(cmd.fields) do
			...
			TrinityAdmin.AutoSize(editBox, 1, 13)
        ...
        sendButton:SetPoint("TOPLEFT", blockFrame, "TOPLEFT", fieldXOffset - 20, -18)
        sendButton:SetText(L["Send"])
		TrinityAdmin.AutoSize(sendButton, 20, 16)
    ...
    while pairIndex <= half do
        ...
        if cmd1 then
            ...
            sendButton1:SetPoint("TOPLEFT", rowFrame, "TOPLEFT", fieldXOffset1 + 20, -18)
            sendButton1:SetText(L["Send"])
			TrinityAdmin.AutoSize(sendButton1, 20, 16)
...
for _, field in ipairs(cmdData.fields) do
    ...
    -- 1) On fixe largeur et hauteur…
    local w = field.width
    local h = 25   -- ou field.height si tu veux le stocker dans tes données
    eb:SetSize(w, h)

    -- 2) …puis on ajuste les 3 textures du template
    if eb.Left then
        eb.Left  :SetSize(8,  h)   -- 8 px de large, h px de haut
        eb.Middle:SetHeight(h)     -- largeur tileable automatique
        eb.Right :SetSize(8,  h)
    end

    table.insert(fieldInputs, eb)
    fieldXOffset = fieldXOffset + w + 10
end

            local sendButton = CreateFrame("Button", nil, rowFrame, "UIPanelButtonTemplate")
            -- sendButton:SetSize(60, 22)
            sendButton:SetPoint("TOPLEFT", rowFrame, "TOPLEFT", fieldXOffset + 20, -18)
            sendButton:SetText(L["Send"])
			TrinityAdmin.AutoSize(sendButton, 20, 16)
            sendButton:SetScript("OnClick", function()
                local args = {}
    ...
    if #singleInputCommands > 0 then
		...
		-- -----------------------------------------------------
		local field = defaultCmd.fields[1]
		-- local w, h = field.width, field.height or 22
		local w = field.width
        local h = 25   -- ou field.height si tu veux le stocker dans tes données
		singleEditBox:SetSize(w, h)
		if singleEditBox.Left then
			singleEditBox.Left:SetSize(8, h)
			singleEditBox.Middle:SetHeight(h)
			singleEditBox.Right:SetSize(8, h)
		end
		-- -----------------------------------------------------
		...
		-- TrinityAdmin.AutoSize(singleEditBox, 20, 13)

        local singleDropdown = CreateFrame("Frame", nil, singleFrame, "TrinityAdminDropdownTemplate")
        singleDropdown:SetPoint("LEFT", singleEditBox, "RIGHT", 10, -2)
		...
		TrinityAdmin.AutoSize(singleSendButton, 20, 16)
