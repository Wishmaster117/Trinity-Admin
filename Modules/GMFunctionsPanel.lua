...
function module:CreateGMFunctionsPanel()
    ...
    navPageLabel:SetText(L["Page"] .. " " .. currentPage .. " / " .. totalPages)
...
if buttonRefs["btnGPS"] then
    buttonRefs["btnGPS"]:SetScript("OnClick", function()
        ...
        if anchor then
			...
			TrinityAdmin.AutoSize(appearEdit, 20, 13)
			...
			TrinityAdmin.AutoSize(btnAppearGo, 20, 16)
           ...
           TrinityAdmin:Print(L["btn_mailbox_anchor_error"])
        end
        ...
        if anchor2 then
            ...
            morphLabel:SetPoint("TOPLEFT", anchor2, "BOTTOMLEFT", 230, -20)
			...
			TrinityAdmin.AutoSize(morphEdit, 20, 13)
			...
			TrinityAdmin.AutoSize(btnMorphGo, 20, 16)
        ...
        if anchorMute then
			...
			-- nameEdit:SetSize(120, 26)
			nameEdit:SetPoint("LEFT", muteDropdown, "RIGHT", 10, 0)
			nameEdit:SetMaxLetters(50)
			nameEdit:SetText(L["Player Name"])  -- par défaut vide
			TrinityAdmin.AutoSize(nameEdit, 20, 13)
			...
			timeEdit:SetText(L["Duration_Mute"])
			TrinityAdmin.AutoSize(timeEdit, 20, 13)			
			...
			reasonEdit:SetSize(140, 26)
			reasonEdit:SetPoint("LEFT", timeEdit, "RIGHT", 10, 0)
			reasonEdit:SetMaxLetters(100)
			reasonEdit:SetText(L["Reason"])
			TrinityAdmin.AutoSize(reasonEdit, 20, 13)	
			...
			TrinityAdmin.AutoSize(btnMuteGo, 20, 16)
			...
			btnMuteGo:SetScript("OnClick", function()
				...
				if option.text == "mute" then
					...
					if (not inputPlayerName or inputPlayerName == "") then
						TrinityAdmin:Print(L["Please enter a Player NAme or Select a Player"])
						return
					end
					if (not inputTime or inputTime == "") then
						TrinityAdmin:Print(L["Please, enter time in minuts."])
						return
					end
					if (not inputReason or inputReason == "") then
						TrinityAdmin:Print(L["Please enter a reason for the mute."])
						return
					end
		...
		TrinityAdmin.AutoSize(displayButton, 20, 16)
		...
		levelEdit:SetText(L["Level"])
		TrinityAdmin.AutoSize(levelEdit, 20, 13)
		...
		btnSetSkill:SetScript("OnClick", function()
			...
			if not selectedSkill then
				TrinityAdmin:Print(L["Error : Please select a skill."])
				return
			end
			local level = levelEdit:GetText()
			if not level or level == "" or level == "Level" then
				TrinityAdmin:Print(L["Error : Please enter a value for Level."])
				return
			end
...
--------------------------------------------------------------------------
	do
		...
		TrinityAdmin.AutoSize(btnDistance, 20, 16)
        ...
        btnDistance:SetScript("OnClick", function()
            ...
            if not targetName then
                TrinityAdmin:Print(L["distance_target_error"])
                return
            end
            local cmd = ".distance "
		...
		TrinityAdmin.AutoSize(editAreaHide, 20, 13)
		...
		TrinityAdmin.AutoSize(btnHideArea, 20, 16)
        ...
        btnHideArea:SetScript("OnClick", function()
            ...
            if not val or val == "" or val == L["Area ID"] then
                TrinityAdmin:Print(L["enter_area_id_hidearea_error"])
                return
            end
        ...
        editAreaShow:SetText(L["Area ID"])
		TrinityAdmin.AutoSize(editAreaShow, 20, 13)
		...
		TrinityAdmin.AutoSize(btnShowArea, 20, 16)
    ...
    ---------------------------------------------------------------
		...
		TrinityAdmin.AutoSize(editSummon, 20, 13)
		...
		TrinityAdmin.AutoSize(btnSummon, 20, 16)
        ...
        btnSummon:SetScript("OnClick", function()
            ...
            if not val or val == "" or val == L["Player Name"] then
               TrinityAdmin:Print(L["enter_player_name_summon_error"])
                return
            end
		...
		TrinityAdmin.AutoSize(editRecall, 20, 13)
		...
		TrinityAdmin.AutoSize(btnRecall, 20, 16)
        ...
        btnRecall:SetScript("OnClick", function()
            ...
            if not val or val == "" or val == L["Player Name"] then
                ...
                local targetName = UnitName("target")
                if not targetName then
                   TrinityAdmin:Print(L["enter_name_or_target_recall_error"])
                    return
                end
    ...
    ---------------------------------------------------------------
		...
		TrinityAdmin.AutoSize(btnBindsight, 20, 16)
        ...
        btnBindsight:SetScript("OnClick", function()
            local targetName = UnitName("target")
            if not targetName then
                TrinityAdmin:Print(L["target_required_bindsight_error"])
                return
            end
            local cmd = ".bindsight"
            SendChatMessage(cmd, "SAY")
		...
		TrinityAdmin.AutoSize(btnUnbindsight, 20, 16)
        ...
        btnUnbindsight:SetScript("OnClick", function()
            local targetName = UnitName("target")
            if not targetName then
                TrinityAdmin:Print(L["target_required_unbindsight_error"])
                return
            end
		...
		-- btnUnbindsight:SetHeight(22)
		TrinityAdmin.AutoSize(btnHonorUpdate, 20, 16)
    ...
    ---------------------------------------------------------------
		...
		TrinityAdmin.AutoSize(editChannel, 20, 13)
		...
		TrinityAdmin.AutoSize(btnSetOwner, 20, 16)
        ...
        btnSetOwner:SetScript("OnClick", function()
            ...
            if not chanName or chanName == "" or chanName == L["Channel"] then
                TrinityAdmin:Print(L["enter_channel_error"])
                return
            end
            ...
            if not state then
                TrinityAdmin:Print(L["check_on_off_error"])
                return
            end
    ...
    ---------------------------------------------------------------
		...
		TrinityAdmin.AutoSize(btnWeather, 20, 16)
        ...
        btnWeather:SetScript("OnClick", function()
            if not weatherDropdown.selected then
                TrinityAdmin:Print(L["select_weather_type_error"])
                return
            end
            if not statusDropdown.selected then
                TrinityAdmin:Print(L["select_status_error"])
                return
            end
    ...
    ---------------------------------------------------------------
		...
		TrinityAdmin.AutoSize(btnShowGrave, 20, 16)
	...
	TrinityAdmin.AutoSize(editGraveID, 20, 16)
	...
	TrinityAdmin.AutoSize(btnLinkGrave, 20, 16)
	...
	btnLinkGrave:SetScript("OnClick", function()
		...
		if graveID == "" or graveID == "Grave ID" then
			TrinityAdmin:Print(L["enter_grave_id_linkgrave_error"])
			return
		end
